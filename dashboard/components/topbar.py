...
def render_topbar(latest_date: str, page_title: str = "Market Overview"):
    ...
    col_left, col_right = st.columns([2.0, 2.0])
    with col_left:
        html_left = f"""<div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap; padding: 2px 0;">
<span style="font-weight: 800; font-size: 1.05rem; color: {t['text_primary']}; letter-spacing: -0.01em;">{page_title}</span>
{badge_html}
</div>"""
        st.markdown(textwrap.dedent(html_left).strip(), unsafe_allow_html=True)
        
    with col_right:
        c_meta, c_btn = st.columns([2.2, 1.4])
        with c_meta:
            html_meta = f"""<div style="display: flex; align-items: center; justify-content: flex-end; gap: 8px; font-size: 0.74rem; font-family: 'JetBrains Mono', monospace; flex-wrap: wrap; height: 100%; padding-top: 4px;">
<div style="color: {t['text_muted']};">
SESSION: <span style="font-weight: 700; color: {t['text_primary']};">{formatted_date}</span>
</div>
<div style="color: {t['text_dim']};">|</div>
<div style="background-color: {t['positive_bg']}; color: {t['positive']}; border: 1px solid {t['positive_border']}; padding: 1px 6px; border-radius: 4px; font-weight: 700; font-size: 0.62rem;">
● CURRENT
</div>
</div>"""
            st.markdown(textwrap.dedent(html_meta).strip(), unsafe_allow_html=True)
