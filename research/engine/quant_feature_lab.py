"""
Phase D & E: Master Point-in-Time Quantitative Factor Engine.
Computes 8 factor families for all 3,492 stocks chronologically (t <= T):
1. Multi-Horizon Returns (1D, 3D, 5D, 10D, 20D, 60D, 120D, 252D) & Momentum
2. Trend & Moving Averages (SMA10..200, EMA10..200, Slopes, Trend Stacking)
3. Volatility, Downside Vol, Realized Vol (5D, 10D, 20D, 60D), ATR, Vol-Adjusted Momentum
4. Drawdowns (from 20D High, 52W High) & 52W Range Position
5. Volume Engine (Ratios 5D/20D/60D, Z-Score, Acceleration, Confirmation Score)
6. Delivery Engine (Delivery %, Delivery Change, Delivery Z-Score)
7. Multi-Horizon Relative Strength (vs Market, Sector, Industry)
8. Point-in-Time Cross-Sectional Z-Scores and Percentile Ranks
"""
...
class QuantFeatureLab:
    def __init__(self, db: Database = None):
        self.db = db or Database()

    def build_all_features(self) -> pd.DataFrame:
        print("\n--- [Phase D & E] Computing Point-in-Time Stock Factors (1.07M rows) ---")
        ...
        bench_map_5d = df_bench.set_index('date')['return_5d'].to_dict() if 'return_5d' in df_bench.columns else {}
        bench_map_20d = df_bench.set_index('date')['return_20d'].to_dict() if 'return_20d' in df_bench.columns else {}
        bench_map_60d = df_bench.set_index('date')['return_20d'].to_dict() if 'return_20d' in df_bench.columns else {}

        df_prices['market_ret_5d'] = df_prices['date'].map(bench_map_5d).fillna(0.0)
        df_prices['market_ret_20d'] = df_prices['date'].map(bench_map_20d).fillna(0.0)
        df_prices['market_rs_5d'] = (df_prices['ret_5d'].fillna(0.0) - df_prices['market_ret_5d']).round(2)
        df_prices['market_rs_20d'] = (df_prices['ret_20d'].fillna(0.0) - df_prices['market_ret_20d']).round(2)

        # Sector & Industry Mean Returns for Relative Strength
        ...
        # 8. Cross-Sectional Z-Scores & Percentile Ranks (Date-by-Date Point-in-Time)
        print("Computing point-in-time cross-sectional percentile ranks...")
        ...
        print(f"Feature engineering complete: {len(df_prices):,} rows with 40+ point-in-time factors.")
        return df_prices
